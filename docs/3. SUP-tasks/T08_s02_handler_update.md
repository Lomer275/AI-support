...
- [ ] Авторизованный клиент получает ответ через pipeline
- [ ] Telegram показывает «печатает...» в течение всего времени ответа
- [ ] При падении pipeline клиент получает fallback-ответ (не стектрейс)
- [ ] Ответы в `WAITING_INN` / `WAITING_PHONE` не изменились (регрессия)
- [ ] `session["inn"]` корректно передаётся в `answer()`
