...
- [x] `search_client_by_inn` возвращает форматированный контекст при наличии документов
- [x] `search_client_by_inn` возвращает `"[Данные не найдены]"` если документов нет
- [x] `get_chat_history` возвращает записи в хронологическом порядке (старые → новые)
- [x] `save_chat_message` сохраняет запись в Supabase
- [x] Ошибки соединения логируются, не бросают исключение наружу
