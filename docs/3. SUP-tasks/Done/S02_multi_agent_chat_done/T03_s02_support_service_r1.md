...
- [x] Все три метода возвращают непустую строку при валидных входах
- [x] Промпты соответствуют нодам `R1 Lawyer`, `R1 Manager`, `R1 Sales` из `Support_new.json`
- [x] `_r1_sales` не получает `r1_lawyer` / `r1_manager` в промпт
- [x] При ошибке OpenAI методы возвращают `None`, не бросают исключение
