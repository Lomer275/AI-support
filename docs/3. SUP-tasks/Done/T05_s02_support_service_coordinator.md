...
- [x] `answer()` возвращает строку (ответ клиенту)
- [x] История сохраняется в `chat_history` после каждого вызова
- [x] Координатор получает историю из `get_chat_history`
- [x] При `None` от любого агента R1/R2 — `answer()` бросает `RuntimeError`
- [x] Невалидный JSON от Координатора не роняет бота — возвращается `FALLBACK_ALINA`
- [x] `switcher="true"` — ответ клиенту всё равно отправляется
