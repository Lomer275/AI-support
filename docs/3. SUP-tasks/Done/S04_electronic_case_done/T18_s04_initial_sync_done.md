...
- Полный прогон: **996/998 сделок синхронизировано**, 2 пропущено (нет ИНН: deal_221712, deal_270492), 0 ошибок
- В Supabase `electronic_case`:
  - 992 строки в `cases` (4 уникальных ИНН у нескольких сделок — upsert по последней)
  - 990/992 с `folder_url`, 976/992 с `contract_number`, 834/992 с `first_hearing_date`
- Обнаружено: поле `debt_amount` (UF_CRM_1614679272145) заполнено у 0/10 сделок — МС не заполняют его, реальные суммы в COMMENTS
- Enum-поля (city, court_region) декодированы через `crm.deal.fields` items list
