...
class SupportService:
    ...
    async def _coordinator(
        ...
        system = (
            ...
            "B) Конкретная дата завершения процедуры — ЗАПРЕЩЕНА. "
            "Допустимо: «ориентировочно ещё несколько месяцев», «процедура идёт по плану».\n"
            "C) КАРТОЧКА КЛИЕНТА — обязательна к использованию. В начале контекста есть блок "
            "«══ КАРТОЧКА КЛИЕНТА ══» с ключевыми фактами: стадия, имущество, суд, дата. "
            ...
            "- «Наша команда уже контролирует это, вам ничего делать не нужно»\n"
            "- «Мы сообщим вам, когда потребуется ваше участие»\n"
            "- «Сохраните это письмо/уведомление и перешлите нам сюда»\n"
            "- «Не предпринимайте никаких действий — мы этим занимаемся»\n"
            ...
            "   - Есть дата завершения? → убрать (правило B)\n"
    ...
    def _build_client_card(
        ...
        # Date of bankruptcy recognition
        m = re.search(
            r"(?:Признать|признать)\s+.*?(?:банкротом|несостоятельным)"
            r"|(?:РЕШИЛ|решил).*?(?:банкротом|несостоятельным)",
            judicial_docs,
        )
        # Use document date if available
        m_date = re.search(r"\d{2}\s+\w+\s+\d{4}\s+года", judicial_docs)
        if not m_date:
            m_date = re.search(r"(\d{2})[.\s](\d{2})[.\s](\d{4})", judicial_docs)
        if m_date:
            facts["Дата признания банкротом"] = m_date.group(0).strip()
