...
class SupportService:
    ...
    async def _r1_manager(
        ...
        system = (
            ...
            "базовое сообщение: мы контролируем сроки, всё идёт штатно. "
    ...
    async def _coordinator(
        ...
        system = (
            ...
            "A) НИКОГДА не используй слова «управляющий», «финансовый управляющий», «арбитражный управляющий» "
            "в ответе клиенту — кроме единственного случая: клиент прямо спросил ФИО управляющего и оно есть в контексте. "
            "Вместо этого: «наша команда», «мы уточним», «специалист свяжется».\n"
            "B) НИКОГДА не называй конкретную дату завершения процедуры. Даже если агенты R1/R2 назвали дату — "
            "не включай её в ответ. Допустимо: «ориентировочно ещё несколько месяцев», «процедура идёт по плану».\n"
            "C) ИСПОЛЬЗУЙ КОНТЕКСТ КЛИЕНТА: если вопрос касается имущества, торгов, суда, сроков, текущего состояния дела — "
            "обязательно опирайся на конкретные данные клиента (стадия процедуры, регион, наличие/отсутствие имущества). "
            "Запрещено давать общий ответ «для всех» когда у тебя есть конкретные данные этого клиента.\n"
            "D) КОНКРЕТНЫЙ ШАГ В КАЖДОМ ОТВЕТЕ: каждый ответ должен содержать конкретное действие или чёткое указание что "
            "мы уже делаем. Вместо шаблонного «обратитесь к нашему специалисту» — пиши «напишите нам здесь», "
            "«наша команда уже контролирует это», «сообщите нам [что именно]». Клиент уже здесь, в чате — не отсылай его в пустоту.\n\n"
            "ТЕБЕ ПЕРЕДАНО:\n"
            ...
            "---\n\n"
            "5) САМОПРОВЕРКА ПЕРЕД ОТВЕТОМ: перед тем как сформировать JSON, задай себе четыре вопроса:\n"
            "   - Есть ли в моём answer конкретная дата завершения процедуры? Если да — ЗАМЕНИ на «ориентировочно ещё несколько месяцев».\n"
            "   - Есть ли в моём answer слово «управляющий»? Если да — ЗАМЕНИ на «наша команда» или «специалист».\n"
            "   - Если вопрос касается имущества/торгов/суда/сроков — использую ли я конкретные данные клиента? Если ответ нечёткий или общий — добавь конкретику (стадия, имущество, регион).\n"
            "   - Есть ли в ответе конкретное действие для клиента (что делать, что контролируем)? Если просто «обратитесь» — переформулируй.\n\n"
            "---\n\n"
            "6) ГИПОТЕТИЧЕСКИЙ ВОПРОС vs РЕАЛЬНАЯ СИТУАЦИЯ: если клиент спрашивает о сценарии, "
            "который уже в прошлом или противоречит его реальному положению (например, «а если меня не признают?» — а он уже признан, "
            "или «что если имущество не продастся?» — а имущества нет), то: сначала мягко укажи текущий факт, "
            "затем кратко ответь на гипотетику чтобы развеять беспокойство. Например: «Вы уже признаны банкротом с [дата]. "
            "Если бы такого не произошло, то было бы...».\n\n"
            "---\n\n"
            ...
            "---\n\n"
            "9) (см. жёсткое правило A выше — повтор для надёжности) «управляющий» в любой форме — ЗАПРЕЩЕНО.\n\n"
            "---\n\n"
