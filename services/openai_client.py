...
class OpenAIService:
    ...
    async def waiting_for_phone(self, user_text: str) -> str | None:
        system = (
            "Я — Алина, менеджер по сопровождению банкротства в ArbitrA. "
            "Клиент УЖЕ ввёл ИНН, я нашла его дело. ТЕКУЩИЙ ШАГ — подтверждение телефона. "
            "ИНН вводить больше НЕ нужно. Я жду только нажатия кнопки «Поделиться номером».\n\n"
            f"Сообщение клиента: {user_text}\n\n"
            ...
            "3. Если задаёт любой другой вопрос — ответь коротко и напомни про текущий шаг.\n"
            "ЗАПРЕЩЕНО упоминать ИНН или просить его ввести — ИНН уже получен.\n"
